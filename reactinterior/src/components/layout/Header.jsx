  ...
  return (
    <header 
      className={`fixed top-0 left-0 w-full z-50 flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 ${
        isScrolled 
          ? "bg-white/90 shadow-md text-amber-900 backdrop-blur-lg py-3 md:py-4" 
          : "bg-white/90 shadow-sm text-amber-900 backdrop-blur-lg py-4 md:py-6"
      }`}
    >
      {/* Logo */}
          ...
          if (link.name === 'Services') {
            return (
              <div key={i} className="services-dropdown relative">
                ...
                {isServicesOpen && (
                  <div className="absolute top-full left-0 mt-2 w-64 bg-white/95 backdrop-blur-lg rounded-xl shadow-2xl border border-amber-200 py-3 z-50">
                    <div className="flex flex-col space-y-1">
                      {services.map((service, index) => (
                        ...
                        >
                          ...
                          <div className="text-xs text-amber-600 mt-1 group-hover:text-amber-700">
                            {service.description}
                          </div>
                        </Link>
                      ))}
      ...
      <div className="hidden md:flex items-center gap-4">
        <span className="hidden lg:inline text-sm text-amber-700 transition-all duration-500">
          +956 789 4823
        </span>
      </div>
      ...
      <div className="flex items-center gap-3 md:hidden">
        <span className="text-sm text-amber-700 transition-all duration-500">
          +956 789 4823
        </span>
        <svg 
          onClick={() => setIsMenuOpen(!isMenuOpen)} 
          className="h-6 w-6 cursor-pointer text-amber-900 transition-all duration-500" 
          fill="none" 
          stroke="currentColor" 
          strokeWidth="2" 
          viewBox="0 0 24 24"
      ...
      {/* Mobile Menu */}
      <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-amber-900 transition-all duration-500 ${
        ...
        <button 
          ...
          onClick={closeAllMenus}
        >
          <svg className="h-6 w-6 text-amber-900" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          if (link.name === 'Services') {
            return (
              <div key={i} className="flex flex-col items-center">
                ...
                {isServicesOpen && (
                  <div className="flex flex-col gap-3 mt-4 pl-4 border-l-2 border-amber-200">
                    {services.map((service, index) => (
                      <Link
                        key={index}
                        to={service.path}
          ...
          return (
            <Link
              ...
              className={`text-lg font-medium transition ${
                location.pathname === link.path ? 'text-amber-600 font-semibold' : 'text-amber-900 hover:text-amber-600'
              }`}
            >
              {link.name}
            </Link>
